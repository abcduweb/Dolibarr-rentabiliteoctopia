...
$evolution = $cacheMois->getEvolution($annee, $params);
...
// Boutons d'export du rapport produits (CSV / PDF)
print '<div style="margin-bottom:20px;display:flex;gap:8px;">';
print '<a href="export.php?type=produits&annee='.$annee.'" class="button" style="background:#16a085;"><i class="fa fa-file-excel"></i> Rapport produits (CSV)</a>';
print '<a href="export.php?type=produits&format=pdf&annee='.$annee.'" class="button" style="background:#c0392b;"><i class="fa fa-file-pdf"></i> Rapport produits (PDF)</a>';
print '</div>';
...
// Calcul du CA frais de port pour le mois (lignes octopia sans fk_product, description 'port Octopia')
$sqlPort = "SELECT COALESCE(SUM(cd.qty * cd.subprice), 0) AS ca_port
            ...
            WHERE o.entity = ".((int)$conf->entity)."
              AND o.is_refunded = 0
              AND cd.fk_product IS NULL
              AND cd.description LIKE '%port%Octopia%'";
$rPort = $db->query($sqlPort);
$caPort = ($rPort && $oP = $db->fetch_object($rPort)) ? (float)$oP->ca_port : 0;

$rows = array(
    array('CA HT produits',       roc_eur($kpis['ca_total'])),
    array('&nbsp;&nbsp;dont CA frais de port refactures',  '<span style="color:#666;font-size:12px;">'.roc_eur($caPort).' (revenu, deja inclus si saisi dans ventes)</span>'),
